...
int main(int argc, char* argv[]) {
  ...
  while (charCheckForESCKey != 27 && webCam.isOpened()){    // loop until ESC key is pressed or webcam is lost
    ...
    cv::threshold(imgGrayScale, imgThreshold, thresholdValue, 255, thresholdType);
    cv::adaptiveThreshold(imgGrayScale, imgAdaptiveThreshold, 255, 
    ...
    cv::namedWindow("Simple Threshold", cv::WINDOW_AUTOSIZE);
    ...
    cv::createTrackbar("Value", "Simple Threshold", &thresholdValue, 255);
    cv::createTrackbar("Type (0-4)", "Simple Threshold", &thresholdType, 4);

    // Show output video results windows
    cv::imshow("imgOriginal", imgOriginal);
    cv::imshow("imgCanny", imgCanny);
    cv::imshow("Simple Threshold", imgThreshold);
    cv::imshow("Adaptive Threshold", imgAdaptiveThreshold);
